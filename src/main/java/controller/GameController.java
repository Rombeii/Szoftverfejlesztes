...
public class GameController {
    ...
    private void finishGame(){
        ...
        if(winner == null){
            winnerLabel.setText("Döntetlenül zárult a játék!");
            log.info("game finished with a tie");
            GameResult result = makeGameResult("döntetlen");
            GsonHelper.appendToGson(result);
        } else if(winner.equals(player1)){
            winnerLabel.setText("Gratulálunk " + player1.getName() + ",nyertél!");
            GameResult result = makeGameResult(player1.getName());
            GsonHelper.appendToGson(result);
            log.info("game is finished, {} won", winner.getName());
        } else if(winner.equals(player2)){
            winnerLabel.setText("Gratulálunk " + player2.getName() + ",nyertél!");
